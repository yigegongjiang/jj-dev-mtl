...
- 【MUST】UI 纯 Swift 代码（AppKit）；MUST NOT Storyboard / XIB / SwiftUI
- 【MUST】完全本地项目 -> 不上架 App Store；MUST NOT 引入签名公证 / 沙盒 / IAP / TestFlight 相关配置与约束
- 【MUST NOT】过度设计：只做当前需求实际用到的最简实现；MUST NOT 引入假想扩展点 / 未来兼容层 / 未使用的抽象 / 企业级架构模式
